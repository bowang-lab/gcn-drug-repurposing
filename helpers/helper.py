...
def load_ransac_graph(dataset, data_path):
    q_RANSAC_graph = np.load(os.path.join(
        data_path, 'graphs', '{}_query_ransac_graph.npy'.format(dataset)))
    x_RANSAC_graph = np.load(os.path.join(
        data_path, 'graphs', '{}_index_ransac_graph.npy'.format(dataset)))
...
def convert_sparse_matrix_to_sparse_tensor(X):
    ...
    return torch.sparse.FloatTensor(i, v, torch.Size(X.shape))
